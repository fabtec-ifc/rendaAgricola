...
@section("body")
    ...
    {{-- Pesquisa e botão de cadastro de Município --}}
    <div class="d-flex justify-content-between mb-3">
        <form action="{{ route('municipio.index') }}" method="get">
            <div class="d-flex gap-2">
                <input placeholder="Pesquisar Município" class="form-control" type="text" name="filtro" id="filtro" value="{{ $filtro }}">
        ...
        <a href="{{ route('municipio.create') }}" class="btn btn-light-green border-green text-green">
    ...
    <table class="table table-striped table-hover">
        ...
        @foreach($municipios as $municipio)
            <tr>
                ...
                <td> {{ $municipio->estado->descricao }} </td>
                <td class="text-center">
                    <a class="btn btn-light-blue btn-sm" href="{{ route('municipio.show', $municipio->id) }}" data-bs-toggle="tooltip" data-bs-title="Ver">
                        <i class="ri-expand-diagonal-line text-blue"></i>
                    </a>
                </td>
                <td class="text-center">
                    <a class="btn btn-light-orange btn-sm" href="{{ route('municipio.edit', $municipio->id) }}" data-bs-toggle="tooltip" data-bs-title="Editar">
                        <i class="ri-edit-box-line text-orange"></i>
                    </a>
                </td>
                <td class="text-center">
                    <form name="form_delete" action="{{ route('municipio.destroy', $municipio->id) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <span data-bs-toggle="modal" data-bs-target="#mostrarModalExclusao" onclick="botaoExcluir(this, `{{ $municipio->descricao }}`, 'Município')">
